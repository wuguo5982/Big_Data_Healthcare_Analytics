...
FROM events
WHERE events.patient_id Not IN (SELECT mortality.patient_id FROM mortality);
-- 2046050
...
FROM events
WHERE patient_id IN (SELECT patient_id FROM mortality);
-- 1029794
...
FROM (SELECT COUNT(event_id) AS event_count FROM alive_events Group BY patient_id) as t1; 
-- 683.1553, 1, 12627
...
FROM (SELECT COUNT(event_id) AS event_count FROM dead_events GROUP BY patient_id) as t1;
-- 1027.7385, 2 16829
...
FROM (SELECT patient_id, COUNT(DISTINCT(time)) as encounter_count FROM alive_events GROUP BY patient_id) as t1;
--18.6955, 9.0, 1, 391
...
FROM (SELECT patient_id, COUNT(DISTINCT(time)) as encounter_count FROM dead_events GROUP BY patient_id) as t1; 
-- 24.8393, 14.0, 1, 375
...
FROM (SELECT patient_id, DATEDIFF(max(time), min(time)) as record_length FROM alive_events GROUP BY patient_id) as t1;
-- 194.7028, 16.0, 0, 3103
...
FROM (SELECT patient_id, DATEDIFF(max(time), min(time)) as record_length FROM dead_events GROUP BY patient_id) as t1;
-- 157.04191, 25.0, 0, 5364
...
FROM alive_events
-- ***** your code below *****
WHERE event_id LIKE 'DIAG%' GROUP BY event_id ORDER BY diag_count DESC LIMIT 5;
...
FROM alive_events
-- ***** your code below *****
WHERE event_id LIKE 'LAB%' GROUP BY event_id ORDER BY lab_count DESC LIMIT 5;
...
FROM alive_events
-- ***** your code below *****
WHERE event_id LIKE 'DRUG%' GROUP BY event_id ORDER BY med_count DESC LIMIT 5;
