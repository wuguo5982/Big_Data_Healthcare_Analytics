...
deadevents_filter = FILTER eventswithmort BY (mortality::label == 1);

deadevents = FOREACH deadevents_filter GENERATE events::patientid AS patientid, events::eventid as eventid, events::value as value, mortality::label as label, DaysBetween(mortality::mtimestamp,events::etimestamp)-30 AS time_difference;
...
aliveevents_filter = FILTER eventswithmort BY (mortality::label is null);
aliveevents_filter = FOREACH aliveevents_filter GENERATE events::patientid AS patientid, events::eventid AS eventid, events::value AS value, 0 AS label, events::etimestamp AS timestamp;

aliveevents_group = GROUP aliveevents_filter BY patientid;
aliveevents_max = foreach aliveevents_group GENERATE group AS patientid, MAX(aliveevents_filter.timestamp) AS max_TS;
aliveevents = JOIN aliveevents_filter BY patientid, aliveevents_max BY patientid;
aliveevents = FOREACH aliveevents GENERATE aliveevents_filter::patientid, aliveevents_filter::eventid AS eventid, aliveevents_filter::value AS value, aliveevents_filter::label AS label, DaysBetween (aliveevents_max::max_TS, aliveevents_filter::timestamp) AS time_difference; 
...
total_patients = UNION deadevents, aliveevents; 
filtered = filter total_patients by (time_difference >= 0 and time_difference <=2000 and value is not null);
...
new_filtered = GROUP filtered BY (patientid, eventid);
featureswithid = FOREACH new_filtered GENERATE group.$0 AS patientid, group.$1 AS eventid, COUNT(filtered.eventid) AS featurevalue;
...
features_merge = JOIN featureswithid BY eventid, all_features BY eventid;
features = FOREACH features_merge GENERATE featureswithid::patientid AS patientid, all_features::idx AS idx, featureswithid::featurevalue AS featurevalue; 
...
features_group = GROUP features BY idx;
maxvalues = FOREACH features_group GENERATE group AS idx, MAX(features.featurevalue) AS maxvalue;
...
features = FOREACH normalized GENERATE features::patientid AS patientid, features::idx AS idx, (float)features::featurevalue/(float)maxvalues::maxvalue AS normalizedfeaturevalue; 
