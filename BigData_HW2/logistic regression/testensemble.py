...
def predict_prob(classifiers, X):
    # Given a list of trained classifiers, predict the probability of positive label.
    # (Return the average obtained from all the classifiers)
    predictions = []
    for i in range(len(classifiers)):
        predictions.append(classifiers[i].predict_prob(X))
    mean_predict = sum(predictions) / len(predictions)
    return mean_predict
    pass
