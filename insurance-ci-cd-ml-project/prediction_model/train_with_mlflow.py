...
def main(alpha, l1_ratio):
    with mlflow.start_run():
        ...
        # Register the model and version in MLflow Model Registry
        from mlflow.tracking import MlflowClient
        client = MlflowClient()
        run_id = mlflow.active_run().info.run_id
        model_uri = f"runs:/{run_id}/model"
        try:
            client.create_registered_model("InsuranceModel")
        except:
            pass  # Model may already exist
        client.create_model_version(name="InsuranceModel", source=model_uri, run_id=run_id)
